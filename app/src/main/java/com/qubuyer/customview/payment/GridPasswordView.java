package com.qubuyer.customview.payment;
...
public class GridPasswordView extends LinearLayout implements PasswordView {
    ...
    private void initAttrs(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        textColor = ta.getColorStateList(R.styleable.gridPasswordView_textColorPayInput);
        if (textColor == null)
            textColor = ColorStateList.valueOf(getResources().getColor(android.R.color.primary_text_light));
        int textSize = ta.getDimensionPixelSize(R.styleable.gridPasswordView_textSizePayInput, -1);
        if (textSize != -1) {
            this.textSize = ConvertUtils.px2sp(textSize);
        }
        lineWidth = (int) ta.getDimension(R.styleable.gridPasswordView_lineWidthPayInput, ConvertUtils.dp2px(1));
        lineColor = ta.getColor(R.styleable.gridPasswordView_lineColorPayInput, DEFAULT_LINECOLOR);
        gridColor = ta.getColor(R.styleable.gridPasswordView_gridColorPayInput, DEFAULT_GRIDCOLOR);
        lineDrawable = ta.getDrawable(R.styleable.gridPasswordView_lineColorPayInput);
        if (lineDrawable == null)
            lineDrawable = new ColorDrawable(lineColor);
        outerLineDrawable = generateBackgroundDrawable();

        passwordLength = ta.getInt(R.styleable.gridPasswordView_passwordLengthPayInput, DEFAULT_PASSWORDLENGTH);
        passwordTransformation = ta.getString(R.styleable.gridPasswordView_passwordTransformationPayInput);
        if (TextUtils.isEmpty(passwordTransformation))
            passwordTransformation = DEFAULT_TRANSFORMATION;
        passwordType = ta.getInt(R.styleable.gridPasswordView_passwordTypePayInput, 0);
        ta.recycle();
