 ...
 * @date 创建时间:2019/6/4
 * @author Susong
 * @description 首页超级返Adapter
 & @version
 */
public class HomeSuperReturnAdapter extends RecyclerView.Adapter {
    private Context mContext;
    private List<HomeGoodEntity> mData;

    public HomeSuperReturnAdapter(Context context, List<HomeGoodEntity> data) {
        mContext = context;
        mData = data;
    }
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(mContext).inflate(R.layout.item_home_super_return_second, parent, false);
        return new HomeSuperReturnSecondVH(view);
    }

    @Override
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof HomeSuperReturnSecondVH) {
            setCommonInfo((HomeSuperReturnSecondVH) holder, position);
        }
    }
    ...
    private void setCommonInfo(HomeSuperReturnSecondVH holder, int position) {
