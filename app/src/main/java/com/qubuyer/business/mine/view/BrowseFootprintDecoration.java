...
public class BrowseFootprintDecoration extends RecyclerView.ItemDecoration {
    private TextPaint mTitleTextPaint;
    private Paint mBackgroundPaint;
    private TitleAttributes mTitleAttributes;

    public BrowseFootprintDecoration(TitleAttributes attributes) {
    ...
    public void onDraw(Canvas c, RecyclerView parent, RecyclerView.State state) {
        ...
        if (parent.getAdapter() == null || !(parent.getAdapter() instanceof MineBrowseFootprintAdapter)) {
            return;
        }
        MineBrowseFootprintAdapter adapter = (MineBrowseFootprintAdapter) parent.getAdapter();
        ...
        for (int i = 0; i < parent.getChildCount(); i++) {
            ...
            if (viewHolder instanceof MineBrowseFootprintVH && titleAttachView(child, parent)) {
                drawTitleItem(c, parent, child, adapter.getData().get(position).getTime());
            }
        }
    }
    ...
    public void onDrawOver(Canvas c, RecyclerView parent, RecyclerView.State state) {
        ...
        if (!(parent.getChildViewHolder(firstView) instanceof MineBrowseFootprintVH)) {
            return;
        }
        ...
        mBackgroundPaint.setColor(mTitleAttributes.mBackgroundColor);
        c.drawRect(parent.getPaddingLeft(), parent.getPaddingTop(), parent.getRight() - parent.getPaddingRight(),parent.getPaddingTop() + mTitleAttributes.mItemHeight, mBackgroundPaint);
        mTitleTextPaint.setTextSize(mTitleAttributes.mTextSize);
        mTitleTextPaint.setColor(mTitleAttributes.mTextColor);
        ...
        c.drawText(adapter.getSortLetters(firstAdapterPosition),
                ...
                StringUtil.getTextBaseLineByCenter(parent.getPaddingTop() + mTitleAttributes.mItemHeight / 2, mTitleTextPaint),
                mTitleTextPaint);
        c.restore();
    }
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
        if (parent.getChildViewHolder(view) instanceof MineBrowseFootprintVH && titleAttachView(view, parent)) {
    ...
    private void drawTitleItem(Canvas c, RecyclerView parent, View child, String letters) {
        ...
        c.drawText(letters, parent.getPaddingLeft() + child.getPaddingLeft() + mTitleAttributes.mTextPadding,
                StringUtil.getTextBaseLineByCenter(textCenterY, mTitleTextPaint), mTitleTextPaint);
    }
    ...
    private boolean titleAttachView(View view, RecyclerView parent) {
        ...
        return null != adapter.getData().get(position)
    ...
    public static class TitleAttributes {
        ...
        public TitleAttributes(Context context) {
            ...
            mTextPadding = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 5, context.getResources().getDisplayMetrics());
            ...
            mBackgroundColor = Color.parseColor("#EEEEEE");
        }
