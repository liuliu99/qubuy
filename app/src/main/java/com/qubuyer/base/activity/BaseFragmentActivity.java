...
public abstract class BaseFragmentActivity extends BaseActivity {
    ...
    public void onNavigationClick(View view) {
        ...
        if (targetFr instanceof BaseFragment) {
            if (!((BaseFragment) targetFr).onNavigationClicked(view)) {
    ...
    public final void push(@NonNull Class<? extends BaseFragment> cls, Map<String, Object> data) {
        push(mEnterAnimation, mExitAnimation, cls, data);
    }

    public final void push(@AnimRes int enterAnimation, @AnimRes int exitAnimation, @NonNull Class<? extends BaseFragment> cls, Map<String, Object> data) {
        if (cls.getName().equals(mCurrFragmentTag)) return;
        FragmentTransaction transaction = mFragmentManager.beginTransaction();
        transaction.setCustomAnimations(enterAnimation, exitAnimation);
        BaseFragment currFr = null;
        if (!TextUtils.isEmpty(mCurrFragmentTag)) {
            currFr = (BaseFragment) mFragmentManager.findFragmentByTag(mCurrFragmentTag);
            if (null != currFr && currFr.isAdded()) {
                transaction.hide(currFr);
            }
        }
        BaseFragment targetFr = (BaseFragment) mFragmentManager.findFragmentByTag(cls.getName());
        if (null != targetFr && targetFr.isAdded()) {
            putData(targetFr.getArguments(), data);
            targetFr.setCurrOper(BaseFragment.OPER_PUSH);
    ...
    protected final void push(@NonNull Class<? extends BaseFragment> cls) {
        push(cls, null);
    }

    public final void popToFragment(@NonNull Class<? extends BaseFragment> cls) {
        if (!TextUtils.isEmpty(mCurrFragmentTag)) {
            BaseFragment currFr = (BaseFragment) getSupportFragmentManager().findFragmentByTag(mCurrFragmentTag);

            if (null != currFr) {
                BaseFragment targetFr = null;
        ...
        if (!TextUtils.isEmpty(mCurrFragmentTag)) {
            ...
            if (null != currFr) {
                ...
                if (cls.getName().equals(mCurrFragmentTag)) {
                    BaseFragment lastNode = currFr.getLastNode();
                    BaseFragment nextNode = currFr.getNextNode();
                    if (null == lastNode) {
                    ...
                    while (null != (targetFr = currFr.getLastNode())) {
                        if (cls.getName().equals(targetFr.getTag())) {
                            BaseFragment lastNode = targetFr.getLastNode();
                            BaseFragment nextNode = targetFr.getNextNode();
                            if (null != lastNode) {
                                lastNode.setNextNode(nextNode);
                            }
                            if (null != nextNode) {
    ...
    public final void pop(@AnimRes int popInAnimation, @AnimRes int popOutAnimation) {
        if (!TextUtils.isEmpty(mCurrFragmentTag)) {
            BaseFragment currFragment = (BaseFragment) mFragmentManager.findFragmentByTag(mCurrFragmentTag);
            if (null != currFragment && !currFragment.isDetached()) {
                BaseFragment last = currFragment.getLastNode();
     ...
     * @param exitAnim  退出动画
     */
    public void switchTo(@NonNull Class<? extends BaseFragment> target, @AnimRes int enterAnim, @AnimRes int exitAnim) {
        ...
        BaseFragment targetFr = (BaseFragment) mFragmentManager.findFragmentByTag(target.getName());
        if (null != targetFr) {
            transaction.show(targetFr);
     ...
     * @param exitAnim  退出动画
     */
    public void switchTo(@NonNull Class<? extends BaseFragment> target, BaseFragment fragmentBaseCompat, @AnimRes int enterAnim, @AnimRes int exitAnim) {
        if (target.getName().equals(mCurrFragmentTag)) return;
        FragmentTransaction transaction = mFragmentManager.beginTransaction();
        transaction.setCustomAnimations(enterAnim, exitAnim);
        BaseFragment currFr;
        ...
        BaseFragment targetFr = fragmentBaseCompat;
        if (fragmentBaseCompat == null) {
            targetFr = (BaseFragment) mFragmentManager.findFragmentByTag(target.getName());
        }
        if (null != targetFr) {
            transaction.show(targetFr);
            mCurrFragmentTag = target.getName();
    ...
    public void switchTo(@NonNull Class<? extends BaseFragment> target) {
        switchTo(target, 0, 0);
    }

    public void switchTo(@NonNull Class<? extends BaseFragment> target, BaseFragment fragmentBaseCompat) {
