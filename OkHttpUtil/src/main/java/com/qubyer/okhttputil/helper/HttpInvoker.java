...
import com.qubyer.okhttputil.builder.OkHttpRequestBuilder;
import com.qubyer.okhttputil.builder.PostFormBuilder;
...
import com.qubyer.okhttputil.json.JsonUtils;
...
//            }
            ...
            if (HttpManager.getInstance().getToken() != null) {
                headParams.put("token", HttpManager.getInstance().getToken());
            }
    ...
    private OkHttpRequestBuilder createOkHttpRequestBuilder() {
        ...
        if (methodType.equals(HTTP_REQUEST_METHOD_POST)) {
            ...
            if (fileParams != null) {
                ...
                requestBuilder = postFormBuilder.params(params).headers(headParams);
            }
    ...
    private Callback createOkHttpCallback(final Map<String, String> params, final String url, final HttpCallback httpCallback, final Class<?> clz, final boolean callbackInUiThread) {
        return new Callback<ServerResponse>() {
            ...
            public void onError(Call call, Exception e, int id) {
                ...
                ServerResponse response = ServerResponse.createErrorResponse(HttpConstant.Status.LOCAL_STATUS_REQUEST_FAIL, e.getMessage());
    ...
    private ServerResponse response2ServerResponse(Response response, Class<?> clz) {
        ...
        try {
            if (response != null && response.isSuccessful()) {
                ...
                if (body.contains("[]") && body.length() < 70 && !clz.isArray()) {
                    body = body.replace("[]", "{}");
                }
